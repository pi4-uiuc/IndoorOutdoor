...
```{r}
## indoor model
plot(as.numeric(indoor_height$Date), indoor_height$mean, col = as.numeric(substr(indoor_height$treatment_name,1,2))/10)

mIn <- lm(mean ~ as.numeric(date)*treatment_name, indoor_height)
summary(mIn)
...
```{r}
Whole_table <- rbind(indoor_height, Outdoors_Height)[,c(1,2,3,5,6)]
